...
generator.data.table <- merge(generator.data.table, fuel.table[, 
  .(Generator.Name, Fuel)], by = "Generator.Name", all.x = TRUE)
...
fuels.to.gens.to.memberships <- initialize_table(Memberships.prototype, 
  nrow(generator.data.table), list(parent_class = "Generator", 
  child_class = "Fuel", collection = "Fuels"))

fuels.to.gens.to.memberships[,parent_object := generator.data.table[,
  Generator.Name]]
...
Objects.sheet <- merge(Objects.sheet, generator.data.table[, 
  .(Generator.Name, Fuel, class = 'Generator')], 
  by.x = c('class', 'name'), by.y = c('class', 'Generator.Name'), all.x = T)
...
#------------------------------------------------------------------------------|
load.to.region.map <- fread(file.path("../InputFiles", map.region.to.load.RE.file))

# add load data file objects to objects .sheet 
# uses load.to.region.map
load.file.to.object <- initialize_table(Objects.prototype, 
  nrow(load.to.region.map), list(class = "Data File", 
...
# uses load.to.region.map
load.filename.to.properties <- initialize_table(Properties.prototype, 
  nrow(load.to.region.map), list(parent_class = "System", 
    child_class = "Data File", collection = "Data Files", 
    parent_object = "System", property = "filename", band_id = 1, value = 0))
...
# uses load.to.region.map
load.to.region.properties <- initialize_table(Properties.prototype, 
  nrow(load.to.region.map), list(parent_class = "System", 
...
load.to.region.properties[, filename := paste0("{Object}",
  load.to.region.map[,RegionName], " Load File Object")]
...
load.part.fact.table <- load.part.fact.table[,
  list(ActivePower.MW = sum(ActivePower.MW)), by = "BusNumber"]
# merge with nodes table
load.part.fact.table <- merge(load.part.fact.table, node.data.table[, 
  .(BusNumber, BusName, RegionName)], by = "BusNumber", all.y = TRUE)
...
lpf.to.node.properties <- load.part.fact.table[,.(BusName, 
  `Load Participation Factor` = LPF)]
...
start.cost.thermal = start.cost[!is.na(MaxOutput.MW), .(Fuel, `Start Cost`, 
  MaxOutput.MW)]
start.cost.na <- merge(start.cost.na, generator.data.table[,.(Generator.Name, 
  Fuel, MaxOutput.MW)], by = "Fuel", all.x = TRUE)

generator.data.table[, MaxOutput.MW.group := ifelse(MaxOutput.MW <= 210, 210, 
  ifelse(MaxOutput.MW <= 500, 500, 610)), by=Generator.Name]

setnames(start.cost.thermal, "MaxOutput.MW", "MaxOutput.MW.group")

start.cost.thermal = merge(start.cost.thermal, generator.data.table[,.
  (Generator.Name, Fuel, MaxOutput.MW.group, MaxOutput.MW)], 
...
start.cost.to.properties <- initialize_table(Properties.prototype, 
  nrow(start.cost), list(parent_class = "System", child_class = "Generator", 
...
for (i in seq(interfaces.files.list)) {
  ...
  interface.names <- fread(file.path("../InputFiles", 
    interfaces.files.list[[i]]['names']))
  interface.properties <- fread(file.path("../InputFiles", 
    interfaces.files.list[[i]]['properties']))
  interface.memberships <- fread(file.path("../InputFiles", 
    interfaces.files.list[[i]]['memberships']))
  interface.coefficients <- fread(file.path("../InputFiles", 
    interfaces.files.list[[i]]['flowcoefs']))
  
  # Add interfaces to objects sheet
  interfaces.to.objects <- initialize_table(Objects.sheet, 
    nrow(interface.names), list(class = "Interface", 
    name = interface.names[,Interface.Name], 
  ...
  interface.to.properties <- initialize_table(Properties.prototype, 
    nrow(interface.names), list(parent_class = 'System', 
    parent_object = 'System', collection = 'Interfaces', 
    child_class = 'Interface', band_id = 1))
  ...
  interface.to.memberships <- initialize_table(Memberships.prototype, 
    nrow(interface.memberships), list(parent_class = "Interface", 
    child_class = "Line", collection = "Lines"))
  
  interface.to.memberships[,parent_object := interface.memberships[,
    Interface.Name]]
  ...
  interface.coefficients.to.props <- initialize_table(Properties.prototype, 
    nrow(interface.coefficients), list(parent_class = "Interface", 
      child_class = "Line", collection = "Lines", band_id = 1, 
      property = "Flow Coefficient", value = -1))
