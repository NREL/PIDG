...
warnings <- file.path(outputfiles.dir,"DataCheck/warnings.txt")
...
fatal.warnings <- file.path(outputfiles.dir,"DataCheck/fatal.warnings.txt")
...
islands <- which(components$csize != max(components$csize)) 

isolated.components <- components$membership[components$membership %in% islands]

island.nodes <- data.table(`Isolated Nodes` = names(isolated.components))

write.csv(island.nodes,  
          file = file.path(outputfiles.dir,
                           "DataCheck/isolated.nodes.csv"))

# Export a report that has:
# 1. Number of connected components (aka islands)
# 2. Unique conn. comp. sizes
# 3. Number of nodes that belong to islands
isolated.nodes.report <- file.path(outputfiles.dir,
                                   "DataCheck/isolated.nodes.report.txt")

sink(isolated.nodes.report)
cat(sprintf("***Summary of connected components in %s database***", choose.db))
cat("\n\n")
cat(sprintf("1. Number of connected components: %1$d",components$no))
cat("\n\n")
cat(sprintf("2. Unique number of nodes per component: %s", 
    paste(unique(components$csize), collapse = ", ")))
cat("\n\n")
cat(sprintf("3. Number of nodes that belong to islands: %d*", nrow(island.nodes)))
cat("\n\n")
...
generator.map[,scenario := ifelse(is.na(scenario),"Scenario: NA",scenario)]
...
pdf(file.path(outputfiles.dir,"DataCheck","region.capcaity.plots.pdf"),
    width = 12, height = 8)
stepi = 0
...
line.map[is.na(resis.scenario),resis.scenario:="Scenario: NA"]

line.map[,reac.scenario:=gsub("{Object}","Scenario: ",reac.scenario,fixed = T)]
line.map[is.na(reac.scenario),reac.scenario:="Scenario: NA"]
...
pdf(file.path(outputfiles.dir,"DataCheck","line.plots.pdf"),
    width = 12, height = 8)
...
for(item in missing.items.list){
  if(nrow(get(item)) > 0){
    ...
    cat("\n\n")
    cat(sprintf("At least one %s ", object.name))
...
# open data check reports
file.show(isolated.nodes.report)
file.show(warnings)
